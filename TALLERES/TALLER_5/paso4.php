...
// 10. Función de búsqueda por título o autor
...
function generarReporteBiblioteca($biblioteca) {
    ...
    $reporte['libros_por_genero'] = array_count_values(array_column($biblioteca, 'genero'));

    // Autor con más libros
    $conteoAutores = array_count_values(array_column($biblioteca, 'autor'));
    arsort($conteoAutores);  // Ordenar para que el autor con más libros esté primero
    $reporte['autor_con_mas_libros'] = array_key_first($conteoAutores);
