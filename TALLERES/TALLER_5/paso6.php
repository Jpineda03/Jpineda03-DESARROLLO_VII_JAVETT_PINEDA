...
// TAREA: Implementar una función que analice el crecimiento de ventas
...
// Función para calcular el crecimiento de ventas entre el primer y último mes
function calcularCrecimiento($ventas) {
    $primeraVenta = $ventas[0];
    $ultimaVenta = end($ventas);
    if ($primeraVenta == 0) {
        return $ultimaVenta > 0 ? 100 : 0;  // Si la primera venta fue 0, todo el crecimiento es 100% si hubo ventas.
    }
    return (($ultimaVenta - $primeraVenta) / $primeraVenta) * 100;
}

// Inicializar variables para almacenar el mayor crecimiento
$mayorCrecimiento = -INF;
$productoConMayorCrecimiento = '';
$regionConMayorCrecimiento = '';

// Calcular el crecimiento de ventas por producto y región
echo "\nCrecimiento de ventas por región y producto (en porcentaje):\n";
foreach ($ventas as $region => $productos) {
    echo "$region:\n";
    foreach ($productos as $producto => $ventasProducto) {
        ...
        // Encontrar el producto y la región con mayor crecimiento
...
echo "$productoConMayorCrecimiento en la región $regionConMayorCrecimiento con un crecimiento de " . number_format($mayorCrecimiento, 2) . "%\n";
